...
for i=1:length(t)-1
  %si sono fatte variare le prime tre settimane poichè in assenza di
  %controllo preventivo gli altri controlli continuano ad esserci ma a
  %causa del forte aumento dei contagi e delle persone con complicanze
  %della malattie i controlli curativi risultano saturarsi a valori più
  %bassi. Questo rispecchia la realtà; infatti, un problema che l'Italia ha
  %dovuto far fronte è stata la carenza di posti e macchinari per la
  %terapia intensiva. Ne consegue che, con maggiori infetti ospedalizzati e
  %in terpia intensiva i controlli curativi risultano meno efficienti e
  %quindi la loro applicazione è meno consistente.
    ...
    T(i+1) = i;
    S(i+1) = S(i)+step*(b-(+d1+beta*E(i)+c*theta*u_Ta)*S(i)+c*(1-lambda)*tau2*A(i)); 
    E(i+1) = E(i)+step*(beta*E(i)*S(i)-(d2+c*theta*tau1*u_Ta+k)*E(i)); 
...
title('Infetti isol. dom. senza u_p');
grid on;
xlabel('tempo, t'); 
ylabel('Casi in isolamento domiciliare');
set(gcf, 'Position',  [0, 250, 500, 400])

figure
plot(t,I1, '-- k','linewidth',2)
title('Infetti osp. nsenza u_p');
grid on;
xlabel('tempo, t'); 
ylabel('Casi Infetti ospedalizzati');
set(gcf, 'Position',  [520, 250, 500, 400])

figure
plot(t,I2, '-- k','linewidth',2);
title('Infetti TI senza u_p');
grid on;
xlabel('tempo, t'); 
ylabel('Casi infetti in TI');
set(gcf, 'Position',  [1050, 250, 500, 400])